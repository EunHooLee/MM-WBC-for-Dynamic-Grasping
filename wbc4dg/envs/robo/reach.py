...
from wbc4dg.envs.robo import MujocoPandaEnv, MujocoPyPandaEnv

# Ensure we get the path separator correct on windows
# MODEL_XML_PATH = os.path.join("fetch", "reach.xml")
MODEL_XML_PATH = "reach.xml" # ... /assets 까지는 BaseRobotEnv 에서 찾는다.

class MujocoPandaReachEnv(MujocoPandaEnv, EzPickle):
    """
    ### Description
    ...
    * v1: the environment depends on `mujoco_py` which is no longer maintained.
    ...
    def __init__(self, reward_type: str = "sparse", **kwargs):
        # initial_qpos: 이부분은 base 위치다. // panda 에 맞게 변경하기
        initial_qpos = {
            "robot0:slide0": 0.4049,
            "robot0:slide1": 0.48,
            "robot0:slide2": 0.0,
        }
        MujocoPandaEnv.__init__(
            self,
...
class MujocoPyPandaReachEnv(MujocoPyPandaEnv, EzPickle):
    def __init__(self, reward_type: str = "sparse", **kwargs):
        initial_qpos = {
            "robot0:slide0": 0.4049,
            "robot0:slide1": 0.48,
